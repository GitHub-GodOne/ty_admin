...
import { tagListApi, tagSaveApi, tagUpdateApi, tagDeleteApi, tagInfoApi } from '@/api/user';
...
const UserTag: React.FC = () => {
  ...
  const fetchList = useCallback(async (page = 1) => {
    setLoading(true);
    try {
      const res = await tagListApi({ page, limit: pagination.pageSize });
  ...
  const handleEdit = async (id: number) => {
    try {
      const info = await tagInfoApi({ id });
      setEditId(id);
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editId) {
        await tagUpdateApi({ id: editId }, values);
      } else {
        await tagSaveApi(values);
      }
  ...
  return (
    <Card title="用户标签" extra={
      ...
      <Modal title={editId ? '编辑标签' : '添加标签'} open={modalOpen}
